...
export default class ReactJkMusicPlayer extends PureComponent {
  ...
  // }
  //
  Spin = () => <span className="loading group">{this.props.icon.loading}</span>

  get iconMap() {
    const { icon } = this.props
    const loadingComponent = <this.Spin /> // использование функционального компонента Spin

    return { ...DEFAULT_ICON, ...icon, loading: loadingComponent }
  }
            ...
            <section className="panel-content">
              ...
              <div className="progress-bar-content">
                <span className="audio-title" title={audioTitle}>
                  ...
                  <span className="tag" title={category}>
                    {category}
                  </span>
                </span>
  ...
  updateMediaSessionMetadata = () => {
    if ('mediaSession' in navigator && this.props.showMediaSession) {
      const { name, cover, singer, category } = this.state
